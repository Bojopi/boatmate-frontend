...
const View: React.FC<ContractProviderProps> = ({contract, contracts, setContracts, toast}) => {
    ...
    const updateResponse = (state: string) => {
        let data = {contractState: state, date: (new Date()).toISOString()};
        setLoadingDone(true);
        markDone(data)
    }
    ...
    const headerContent = (
        <div className='flex items-center gap-5'>
            ...
            <div className='w-auto h-7 px-3 py-0.5 bg-green-100 rounded-md justify-center items-center gap-2.5 inline-flex'>
                <p className='text-green-800 text-xs font-semibold' >In Progress</p>
            </div>
        </div>
    )
    ...
    const footerContent = (
        <div className="flex items-center justify-between">
            ...
            <Button type="button" label="Mark Done" severity="warning" disabled={loadingDone} icon={`pi ${loadingDone ? "pi-spin pi-spinner" : "pi-check"}`} iconPos="right" onClick={() => updateResponse('FINISHED')} className="rounded-3xl text-sm px-5 py-3" />
        </div>
    );
