...
const InboxPage: React.FC<InboxProps> = ({contractId}) => {
  ...
  const getAddressMap = async (lat: number, lng: number, item: any) => {
    try {
    ...
    try {
      ...
      if(response.status == 200) {
        if(response.data.contracts.length > 0) {
          response.data.contracts.map(async (item: any) => {
            item['address'] = await getAddressMap(Number(item.provider_lat), Number(item.provider_lng), item['address']);
          })
        }
  ...
  const selectContract = (idContract: number) => {
    ...
    if(contracts.length > 0) {
      const item = contracts.find((contract: ContractCustomer) => contract.id_contract === idContract)
  ...
  useEffect(() => {
    if(selectedContract !== null) {
      socket.emit('join-room', {id: selectedContract.id_contract})
    }
  }, [selectedContract])
  ...
  const sendMessages = (e: any) => {
    ...
    const data = {
      ...
      customerId: Number(router.query.idCustomer)
    }
                    ...
                    <div className='w-full'>
                      ...
                      <div className='w-full flex flex-col gap-1 mt-1'>
                        <p className={`text-sm leading-tight ${setColorStatus(item.contract_state)}`} style={{textTransform: 'capitalize'}}>{(item.contract_state).toLowerCase()}</p>
                          {
                            item.address !== null ?
                              <p className='text-sm text-neutral-600 font-light leading-tight'>{item.address}</p>
                    ...
                    <div>
                      <div className='w-full flex items-end gap-5'>
                        ...
                        <div className='w-full flex items-center justify-between'>
                          <div>
                            <p className='text-sm'><span className='text-zinc-500'>{formatDateDash(String(selectedContract.contract_date))}</span> - <span className='text-cyan-600 font-light'>{selectedContract.address !== null ? selectedContract.address : 'No address'}</span></p>
                            <p className='text-lg'>{selectedContract.provider_name}</p>
                          </div>
                      ...
                      <div className={`${loadingChat ? 'w-full h-full flex justify-center z-20 items-center' : 'hidden'}`}>
                      ...
                      </div>
                        {
                          ...
                          storedMessage.map((store: any, i: number) => (
                            <div key={i} className={`flex flex-col ${store.providerId ? 'mr-auto items-start' : 'ml-auto items-end'}`}>
                              <p className={`w-auto text-[10px] text-gray-400 ${store.providerId ? 'text-start': 'text-end'}`}>{formatDateMessage(String(new Date(store.message_date)))}</p>
                              <div className={`py-2 px-3 rounded-xl w-auto text-sm ${store.providerId ? 'bg-gray-200' : 'bg-sky-300'}`}>{store.message_text}</div>
                            </div>
                          ))
